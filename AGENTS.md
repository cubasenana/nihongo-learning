...
- 词汇进度只卡 `📖` 词（课文出现），其余按 ★ 难度分层用 Anki 长线复习。
- AI 讲解语法前，必须联网搜索 Bunpro 对应条目做交叉校验；无法访问时必须明说“未能校验”。
- 查词释义以 MOJI 辞書为准，词表范围以 `resources/jlpt-vocab-n5n4.csv` 为准，不使用标日词表。
...
5. `📖` 词日→中识别是进入下一课的进度门槛；语法产出和场景写作是掌握度建议，不和进度门槛混同。
...
1. `gate-check.md`：只审查，不讲课；P0/P1 阻断。
